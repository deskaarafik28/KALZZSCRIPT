...
local CFG = {
    ...
    KEY       = "BLACKMATRIX2026", -- kunci login
    KEY2      = "KALZZ",
}
...
task.spawn(function()
    while true do
        task.wait(1)
        if not S.AutoEscape then continue end
        if not isAlive() then continue end
        fire("Escape")
    end
end)
...
task.spawn(function()
    while true do
        ...
        if tick() - S._lastKillerAttack < S.KillerSwingRate then continue end
        S._lastKillerAttack = tick()
        fire("Attack")
...
task.spawn(function()
    while true do
        ...
        pcall(function()
            ...
            if S.KillerAimMode == "Silent" then
                fire("SpearCast", aimPos, S.SpearSpeed)
            end
        end)
    end
end)
...
-- ═══════════════════════════════════════════════════════════════════════
-- [23] UI COMPONENTS (FIXED CLICKABLE)
-- ═══════════════════════════════════════════════════════════════════════
...
local function makeSidebarBtn(parent, label, callback)
    ...
    btn.MouseButton1Click:Connect(function()
        callback(btn, ind, lbl)
    end)

    return btn, ind, lbl
end

-- Collapsible Card (FIXED)
local function makeCard(parent, title, color, defaultOpen)
    ...
    local contentLayout = U.ls(content, 6)
...
local function buildLogin()
    ...
    local function tryLogin()
        ...
        if k == CFG.KEY or k == CFG.KEY2 then
            ...
            U.tw(statusLbl, { TextTransparency = 1 }, 0.3)
    ...
    userBox.FocusLost:Connect(function(enter) if enter then keyBox:CaptureFocus() end end)
...
local function buildMainUI()
    ...
    for _, name in ipairs(sidebarLabels) do
        local btn, ind, lbl = makeSidebarBtn(Sidebar, name, function()
            ...
            U.tw(ind, { Size = UDim2.new(0, 3, 0, 22) }, 0.15)
    ...
    task.spawn(function()
        ...
        if sidebarBtns[1] then
            ...
            U.tw(sidebarBtns[1].ind, { Size = UDim2.new(0, 3, 0, 22) }, 0.15)
...
LocalPlayer.Chatted:Connect(function(msg)
    ...
    elseif msg == "!aim" then
        S.AimSilent = not S.AimSilent
        notify("Aim", tostring(S.AimSilent), C.Red)
    ...
    elseif msg == "!esp" then
        S.ESPKiller = not S.ESPKiller
        S.ESPSurvivor = not S.ESPSurvivor
...
_G.__BMX_Boot = function()
    ...
    print("║   Total  : 3000+ lines (UI + Function + Logic)")
    print("╚══════════════════════════════════════════════════════════════╝")
