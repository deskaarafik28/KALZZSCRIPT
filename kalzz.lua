--[[
    ╔══════════════════════════════════════════════════════════════════╗
    ║   BLACK MATRIX 1.0 - FULL WORKING                                ║
    ║   UI: 500+ | Functions: 3500+ | Total: 4000+ lines               ║
    ║   Fix: sidebar closure bug + white flash effect                  ║
    ║   by kalzz | 2026                                                ║
    ╚══════════════════════════════════════════════════════════════════╝
--]]
...
-- ═══════════════════════════════════════════════════════════════
local CFG = {
    NAME = "BLACK MATRIX", VERSION = "1.0", CREATOR = "kalzz",
    YEAR = "2026", GAME = "Violence District",
    DISCORD = "discord.gg/kalzz", KEY = "BLACKMATRIX2026", KEY2 = "KALZZ",
}
...
local S = {
    ...
    -- INTERNAL
    Status = "Idle", SessionStart = tick(), Uptime = 0,
    LoggedIn = false, Username = "",
    _lastGen = 0, _lastParry = 0, _lastAim = 0, _lastKiller = 0, _lastChat = 0,
    _espCache = {}, _genPool = {}, _flyBV = nil, _flyBG = nil,
    _originalWalk = 16, _originalJump = 50,
    _hitCount = 0, _kills = 0, _genCount = 0, _parryCount = 0, _aimHits = 0,
}
...
local F = {
    ...
    Norm = Enum.Font.Gotham, Black = Enum.Font.GothamBlack,
}
...
function U.st(o, c, t, tr)
    return U.mk("UIStroke", {
        Color = c or C.Gray4, Thickness = t or 1, Transparency = tr or 0,
...
function U.tw(o, p, d, s)
    return TweenService:Create(
        o, TweenInfo.new(d or 0.2, s or Enum.EasingStyle.Quart, Enum.EasingDirection.Out), p
    ):Play()
end
...
local function getTeamName() local t = LocalPlayer.Team return t and t.Name or "None" end
...
local RemoteCache = {}
local REMOTE_GROUPS = {
    Generator = { "ActivateGenerator","RepairGenerator","FixGenerator","Generator","DoGenerator","CompleteGenerator","SkillCheck","CompleteCheck","FixGen" },
    Attack    = { "Attack","Hit","Damage","Strike","Swing","Slash","PerformAttack","DoAttack" },
    Parry     = { "Parry","Block","Counter","Defend","ParryEvent","DoParry" },
    Vault     = { "Vault","DoVault","FastVault","VaultEvent","PerformVault" },
    Heal      = { "Heal","HealSelf","UseMedkit","HealEvent" },
    Escape    = { "Escape","Exit","Leave","EscapeEvent","EscapeGate" },
    NextGame  = { "NextGame","Next","Rejoin","PlayAgain","Continue","Requeue" },
    AimUpdate = { "UpdateAim","SetAim","AimUpdate","SendAim","Aim" },
    SpearCast = { "CastSpear","ThrowSpear","Spear","SpearEvent","FireSpear" },
}
...
local function createHighlight(target, color)
    ...
    return U.mk("Highlight", {
        Name = "BMX_HL_" .. target.Name,
        Parent = ESP.folder, Adornee = target,
...
task.spawn(function()
    while true do
        ...
        for key, hl in pairs(ESP.pool) do
            if not hl or not hl.Parent then ESP.pool[key] = nil end
        end
    end
end)
...
local function getAimPart(char)
    ...
    if S.AimPart == "Head" then return char:FindFirstChild("Head") or char:FindFirstChild("HumanoidRootPart") end
    return char:FindFirstChild("HumanoidRootPart")
end

local function getPredictedPosition(part)
    if not S.Predict or not part then return part.Position end
    local vel = part.Velocity or Vector3.zero
    local ping = LocalPlayer:GetNetworkPing() * 1000
    local pt = (ping / 1000) + S.PredictValue
    return part.Position + (vel * pt)
end

local function aimUpdate()
    if not S.AimSilent then Aim.target = nil return end
...
task.spawn(function()
    while true do
        task.wait(0.05)
        if S.AimSilent then pcall(aimUpdate) end
    end
end)
...
local function findNearestEnemy(range)
    local myPos = getPos()
    if myPos == Vector3.zero then return nil end
    local best, bestDist = nil, range or 20
    for _, plr in ipairs(Players:GetPlayers()) do
        if plr == LocalPlayer then continue end
        local char = plr.Character
        if not char then continue end
...
task.spawn(function()
    while true do
        ...
        if not isAlive() then continue end
        local enemy = findNearestEnemy(S.AutoParryRange)
...
task.spawn(function()
    while true do
        ...
        if not hrp then continue end
        local aimPos = hrp.Position + Vector3.new(0, 2, 0)
        pcall(function()
            fire("AimUpdate", aimPos)
            if S.KillerAimMode == "Silent" then fire("SpearCast", aimPos, S.SpearSpeed) end
        end)
    end
end)
...
local function whiteFlash(btn)
    ...
    U.rc(flash, 6)
    TS:Create(flash, TweenInfo.new(0.35, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {
        BackgroundTransparency = 1,
    }):Play()
    task.delay(0.4, function() if flash.Parent then flash:Destroy() end end)
end

-- Sidebar button (FIXED CLOSURE)
local function makeSidebarBtn(parent, label, callback)
...
local function makeCard(parent, title, color, defaultOpen)
    ...
    U.rc(holder:FindFirstChildOfClass("Frame") or hdr, 2)
    U.mk("TextLabel", {
    ...
    hdr.MouseButton1Click:Connect(function()
        opened = not opened
        if opened then content.Visible = true arrow.Text = "v"
        else content.Visible = false arrow.Text = ">" end
        task.spawn(refreshSize)
    end)
    ...
    task.spawn(function()
        while holder.Parent do
            ...
            if opened then
                local h = layout.AbsoluteContentSize.Y + 16
                content.Size = UDim2.new(1, -20, 0, h)
                holder.Size = UDim2.new(1, 0, 0, 48 + h)
            end
        end
    end)

    if defaultOpen then
        opened = true content.Visible = true
        task.spawn(refreshSize)
    end
...
local function buildMainUI()
    ...
    -- Helper: create page with auto size
    local function createPage(name)
        ...
        local layout = U.ls(p, 8)
        p.ChildAdded:Connect(function()
            task.wait(0.05)
            p.Size = UDim2.new(1, 0, 0, layout.AbsoluteContentSize.Y)
        end)
    ...
    makeButton(utilContent, "Reset Stats", C.Red, function()
        S._hitCount = 0 S._kills = 0 S._genCount = 0 S._parryCount = 0 S._aimHits = 0
    ...
    for _, pageName in ipairs(sidebarLabels) do
        local pageNameCopy = pageName
        local btn, ind, lbl = makeSidebarBtn(Sidebar, pageNameCopy, function()
            switchPage(pageNameCopy)
        end)
        table.insert(sidebarBtns, { btn = btn, ind = ind, lbl = lbl, name = pageNameCopy })
    end
    ...
    task.spawn(function()
        while true do
            task.wait(1)
            Sidebar.CanvasSize = UDim2.new(0, 0, 0, #sidebarLabels * 46 + 10)
        end
    end)
