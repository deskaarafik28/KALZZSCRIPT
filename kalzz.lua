--[[
    ╔══════════════════════════════════════════════════════════════════════╗
    ║   BLACK MATRIX 1.0 - FULL WORKING v2                                 ║
    ║   Game  : Violence District                                          ║
    ║   Fix   : sidebar clickable 100%                                     ║
    ║   Total : 4000+ lines                                                ║
    ║   by kalzz | 2026                                                    ║
    ╚══════════════════════════════════════════════════════════════════════╝
--]]
...
local MAIN_H = math.floor(390 * UI_SCALE)
...
-- ═══════════════════════════════════════════════════════════════════════
local S = {
    AutoGenerator = false, GenMode = "Perfect", GenBoost = false, AutoSkillCheck = false,
    AimSilent = false, AimTarget = "Killer", AimPart = "Head", AimFOV = 500,
    AimSmooth = 0.15, AimVisibleOnly = false, ZigzagDetect = true,
    Predict = true, PredictValue = 0.15, TOF = false, TOFSpeed = 800,
    FastVault = false, AntiFallSlow = false, SpeedBoost = false, SpeedValue = 24,
    AutoParry = false, AutoParryRange = 20, InfiniteStamina = false, AutoEscape = false,
    KillerAim = false, KillerAimMode = "Veil", SpearGravity = 100, SpearSpeed = 200,
    KillerPredict = 100, KillerAutoAttack = false, KillerInfiniteLunge = false,
    KillerNoStun = false, KillerAttackRange = 15, KillerSwingRate = 0.4,
    ESPKiller = false, ESPSurvivor = false, ESPGenerator = false,
    ESPHook = false, ESPPallet = false, ESPOutlineOnly = true,
    ESPTransparency = 0.3, ESPDistance = false,
    AntiAFK = true, AntiStun = false, AntiBlind = false, InstantHeal = false,
    Fullbright = false, LowGraphics = false, NoClip = false, Fly = false,
    FlySpeed = 60, InfiniteJump = false, AutoChat = false, ChatMessage = "auto farm by kalzz",
    Status = "Idle", SessionStart = tick(), Uptime = 0, LoggedIn = false, Username = "",
    _lastGen = 0, _lastParry = 0, _lastKiller = 0, _lastChat = 0,
    _flyBV = nil, _flyBG = nil, _originalWalk = 16,
    _hitCount = 0, _genCount = 0, _parryCount = 0, _aimHits = 0,
}
...
-- ═══════════════════════════════════════════════════════════════════════
local C = {
    BG0 = Color3.fromRGB(0,0,0), BG1 = Color3.fromRGB(15,15,18),
    BG2 = Color3.fromRGB(22,22,26), BG3 = Color3.fromRGB(32,32,38),
    BG4 = Color3.fromRGB(45,45,52), BG5 = Color3.fromRGB(60,60,70),
    White = Color3.fromRGB(255,255,255), OffWhite = Color3.fromRGB(230,230,235),
    Gray1 = Color3.fromRGB(180,180,190), Gray2 = Color3.fromRGB(130,130,145),
    Gray3 = Color3.fromRGB(90,90,105), Gray4 = Color3.fromRGB(60,60,72),
    Cyan = Color3.fromRGB(0,200,255), Green = Color3.fromRGB(60,220,130),
    Red = Color3.fromRGB(255,80,100), Yellow = Color3.fromRGB(255,200,80),
    Purple = Color3.fromRGB(170,120,255), Pink = Color3.fromRGB(255,120,200),
    Orange = Color3.fromRGB(255,150,70), Blue = Color3.fromRGB(80,140,255),
}

local F = { Bold = Enum.Font.GothamBold, Med = Enum.Font.GothamMedium, Norm = Enum.Font.Gotham, Black = Enum.Font.GothamBlack }
...
function U.st(o, c, t, tr)
    return U.mk("UIStroke", { Color = c or C.Gray4, Thickness = t or 1, Transparency = tr or 0, ApplyStrokeMode = Enum.ApplyStrokeMode.Border }, o)
end
function U.ls(o, s, d)
    return U.mk("UIListLayout", { Padding = UDim.new(0, s or 6), SortOrder = Enum.SortOrder.LayoutOrder, FillDirection = d or Enum.FillDirection.Vertical }, o)
end
function U.pd(o, t, b, l, r)
    return U.mk("UIPadding", { PaddingTop = UDim.new(0, t or 0), PaddingBottom = UDim.new(0, b or 0), PaddingLeft = UDim.new(0, l or 0), PaddingRight = UDim.new(0, r or 0) }, o)
end
...
local RG = {
    ...
    NextGame  = { "NextGame","Next","Rejoin","PlayAgain" },
    AimUpdate = { "UpdateAim","SetAim","AimUpdate","SendAim","Aim" },
    SpearCast = { "CastSpear","ThrowSpear","Spear","SpearEvent" },
        ...
        if obj:IsA("RemoteEvent") or obj:IsA("RemoteFunction") then
            for _, list in pairs(RG) do
                for _, name in ipairs(list) do
                    if obj.Name == name then RC[name] = obj n = n + 1 end
                end
            end
        end
    end
...
local function notify(title, msg, color, dur)
    color = color or C.Cyan dur = dur or 3
    local f = U.mk("Frame", {
        Size = UDim2.new(0, 270, 0, 60), Position = UDim2.new(1, 12, 0, 12),
        BackgroundColor3 = C.BG2, BackgroundTransparency = 0.1, BorderSizePixel = 0, Parent = nGui,
    })
    U.rc(f, 10) U.st(f, color, 1, 0.3)
    U.mk("Frame", { Size = UDim2.new(0,3,1,-14), Position = UDim2.new(0,7,0,7), BackgroundColor3 = color, BorderSizePixel = 0, Parent = f })
    U.mk("TextLabel", { Size = UDim2.new(1,-22,0,20), Position = UDim2.new(0,16,0,8), BackgroundTransparency = 1, Text = title, TextColor3 = C.White, Font = F.Bold, TextSize = FS.Body, TextXAlignment = Enum.TextXAlignment.Left, Parent = f })
    U.mk("TextLabel", { Size = UDim2.new(1,-22,0,24), Position = UDim2.new(0,16,0,28), BackgroundTransparency = 1, Text = msg, TextColor3 = C.Gray2, Font = F.Norm, TextSize = FS.Small, TextWrapped = true, TextXAlignment = Enum.TextXAlignment.Left, TextYAlignment = Enum.TextYAlignment.Top, Parent = f })
...
U.safe(function() ESP.folder = U.mk("Folder", { Name = "BMX_ESP", Parent = Workspace }) end)

local function createHL(target, color)
    if not ESP.folder or not target then return nil end
    return U.mk("Highlight", {
        Name = "BMX_HL", Parent = ESP.folder, Adornee = target,
        FillColor = color, OutlineColor = color,
        FillTransparency = S.ESPTransparency, OutlineTransparency = 0.15,
...
task.spawn(function()
    while true do
        ...
        for _, plr in ipairs(Players:GetPlayers()) do
            ...
            local color = C.White
            if S.ESPKiller and isKiller(plr) then shouldESP = true color = C.Red
            elseif S.ESPSurvivor and isSurvivor(plr) then shouldESP = true color = C.Green end
            ...
            if shouldESP then
                if existing and existing.Parent then
                    ...
                    ESP.pool[key] = createHL(char, color)
                end
            else
        ...
        if S.ESPGenerator then
            ...
            for i, gen in ipairs(gens) do
                ...
                if not ESP.genPool[key] or not ESP.genPool[key].Parent then
                    ESP.genPool[key] = createHL(gen, C.Yellow)
                else
...
local function getClosestTarget(filter)
    ...
    for _, plr in ipairs(Players:GetPlayers()) do
        ...
        if dist < bestDist and dist <= S.AimFOV then
            ...
            if onScreen then best = plr bestDist = dist end
        end
    end
    return best
end
        ...
        if S.AimSilent then
            ...
            if target and target.Character then
                ...
                if part then
                    ...
                    local pt = (ping / 1000) + S.PredictValue
                    local aimPos = part.Position + (part.Velocity * pt)
                    local dir = (aimPos - Camera.CFrame.Position).Unit
                    pcall(function() fire("AimUpdate", aimPos, dir) end)
...
RunService.Heartbeat:Connect(function()
    ...
    elseif not S.FastVault then h.WalkSpeed = S._originalWalk end
    if S.FastVault then
        local st = h:GetState()
        if st == Enum.HumanoidStateType.Climbing or st == Enum.HumanoidStateType.PlatformStanding then
            h.WalkSpeed = 45
        end
    end
end)
...
task.spawn(function()
    while true do
        ...
        if S.GenMode == "Perfect" or S.AutoSkillCheck then fire("SkillCheck", 1) end
    end
end)
...
task.spawn(function()
    while true do
        ...
        if not isAlive() then continue end
        if tick() - S._lastParry < 0.25 then continue end
        S._lastParry = tick()
        fire("Parry")
...
task.spawn(function()
    while true do
        ...
        if S.AutoChat and tick() - S._lastChat > 45 then
            pcall(function()
                ReplicatedStorage.DefaultChatSystemChatEvents.SayMessageRequest:FireServer(S.ChatMessage, "All")
            end)
            S._lastChat = tick()
        end
    end
...
local function whiteFlash(btn)
    ...
    local flash = U.mk("Frame", {
        Name = "FlashFX", Size = UDim2.new(1, 0, 1, 0),
        BackgroundColor3 = C.White, BackgroundTransparency = 0.5,
        BorderSizePixel = 0, ZIndex = 50, Parent = btn,
    })
    U.rc(flash, 6)
    local tw = TweenService:Create(flash, TweenInfo.new(0.3, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), { BackgroundTransparency = 1 })
    tw:Play()
    tw.Completed:Connect(function() if flash and flash.Parent then flash:Destroy() end end)
end
...
local function makeCard(parent, title, color, defaultOpen)
    ...
    local holder = U.mk("Frame", {
        ...
        BackgroundTransparency = 0.15, BorderSizePixel = 0, ClipsDescendants = false, Parent = parent,
    })
    U.rc(holder, 8) U.st(holder, C.Gray4, 1, 0.4)

    local hdr = U.mk("TextButton", {
        Size = UDim2.new(1, 0, 0, 44), BackgroundTransparency = 1,
        Text = "", AutoButtonColor = false, ZIndex = 5, Parent = holder,
    })

    local bar = U.mk("Frame", {
        Size = UDim2.new(0, 4, 0, 20), Position = UDim2.new(0, 12, 0, 12),
        BackgroundColor3 = color, BorderSizePixel = 0, Parent = hdr,
    })
    U.rc(bar, 2)

    U.mk("TextLabel", {
        Size = UDim2.new(1, -70, 1, 0), Position = UDim2.new(0, 24, 0, 0),
        BackgroundTransparency = 1, Text = title, TextColor3 = C.OffWhite,
        Font = F.Bold, TextSize = FS.H3, TextXAlignment = Enum.TextXAlignment.Left, Parent = hdr,
    })

    local arrow = U.mk("TextLabel", {
        Size = UDim2.new(0, 24, 1, 0), Position = UDim2.new(1, -34, 0, 0),
        BackgroundTransparency = 1, Text = defaultOpen and "v" or ">",
        TextColor3 = C.Gray2, Font = F.Bold, TextSize = 14, Parent = hdr,
    })

    local content = U.mk("Frame", {
        Size = UDim2.new(1, -20, 0, 0), Position = UDim2.new(0, 10, 0, 48),
        BackgroundTransparency = 1, Visible = false, Parent = holder,
    })
    ...
    hdr.MouseButton1Click:Connect(function()
        whiteFlash(hdr)
        opened = not opened
        if opened then content.Visible = true arrow.Text = "v" else content.Visible = false arrow.Text = ">" end
        task.spawn(refresh)
    end)

    if defaultOpen then
        opened = true content.Visible = true task.spawn(refresh)
    end
...
local function makeToggle(parent, label, key, callback)
    local h = U.mk("Frame", {
        Size = UDim2.new(1, 0, 0, 44), BackgroundColor3 = C.BG3,
    ...
    U.mk("TextLabel", {
        ...
        BackgroundTransparency = 1, Text = label, TextColor3 = C.OffWhite,
        Font = F.Med, TextSize = FS.Body, TextXAlignment = Enum.TextXAlignment.Left, Parent = h,
    })
    ...
    clk.MouseButton1Click:Connect(function()
        ...
        S[key] = not S[key]
        local on = S[key]
        U.tw(tr, { BackgroundColor3 = on and C.White or C.BG5, BackgroundTransparency = on and 0 or 0.3 }, 0.2)
        U.tw(dot, { Position = on and UDim2.new(1, -20, 0, 3) or UDim2.new(0, 3, 0, 3), BackgroundColor3 = on and C.BG0 or C.Gray2 }, 0.2)
...
local function makeSlider(parent, label, key, mn, mx, df, sfx, callback)
    sfx = sfx or ""
    local h = U.mk("Frame", {
        Size = UDim2.new(1, 0, 0, 58), BackgroundColor3 = C.BG3,
        BackgroundTransparency = 0.2, BorderSizePixel = 0, Parent = parent,
    })
    U.rc(h, 8)

    U.mk("TextLabel", {
        Size = UDim2.new(0.5, 0, 0, 18), Position = UDim2.new(0, 14, 0, 6),
        BackgroundTransparency = 1, Text = label, TextColor3 = C.Gray1,
        Font = F.Med, TextSize = FS.Small, TextXAlignment = Enum.TextXAlignment.Left, Parent = h,
    })

    local vl = U.mk("TextLabel", {
        Size = UDim2.new(0.5, -14, 0, 18), Position = UDim2.new(0.5, 0, 0, 6),
        BackgroundTransparency = 1, Text = tostring(df) .. sfx, TextColor3 = C.White,
        Font = F.Bold, TextSize = FS.Small, TextXAlignment = Enum.TextXAlignment.Right, Parent = h,
    })

    local tr = U.mk("TextButton", {
        Size = UDim2.new(1, -28, 0, 6), Position = UDim2.new(0, 14, 0, 38),
        BackgroundColor3 = C.BG5, BorderSizePixel = 0, Text = "",
        AutoButtonColor = false, Parent = h,
    })
    U.rc(tr, 3)

    local pct = (df - mn) / (mx - mn)
    local fl = U.mk("Frame", { Size = UDim2.new(pct, 0, 1, 0), BackgroundColor3 = C.White, BorderSizePixel = 0, Parent = tr })
    U.rc(fl, 3)
    local kn = U.mk("Frame", { Size = UDim2.new(0, 14, 0, 14), Position = UDim2.new(pct, -7, 0.5, -7), BackgroundColor3 = C.White, BorderSizePixel = 0, Parent = tr })
    U.rc(kn, 7)
...
local function makeDropdown(parent, label, options, key, callback)
    local h = U.mk("Frame", {
        Size = UDim2.new(1, 0, 0, 66), BackgroundColor3 = C.BG3,
    ...
    U.mk("TextLabel", {
        Size = UDim2.new(1, -24, 0, 18), Position = UDim2.new(0, 14, 0, 6),
        BackgroundTransparency = 1, Text = label, TextColor3 = C.Gray1,
        Font = F.Med, TextSize = FS.Small, TextXAlignment = Enum.TextXAlignment.Left, Parent = h,
    })
    ...
    local sel = U.mk("TextButton", {
        ...
        Text = "", AutoButtonColor = false, Parent = h,
    })
    U.rc(sel, 6)

    local selLbl = U.mk("TextLabel", {
        Size = UDim2.new(1, -40, 1, 0), Position = UDim2.new(0, 10, 0, 0),
        BackgroundTransparency = 1, Text = tostring(S[key]) or options[1],
        TextColor3 = C.OffWhite, Font = F.Med, TextSize = FS.Small,
        TextXAlignment = Enum.TextXAlignment.Left, Parent = sel,
    })

    U.mk("TextLabel", {
        Size = UDim2.new(0, 20, 1, 0), Position = UDim2.new(1, -22, 0, 0),
        BackgroundTransparency = 1, Text = "v", TextColor3 = C.Gray2,
        Font = F.Bold, TextSize = FS.Small, Parent = sel,
    })
    ...
    sel.MouseButton1Click:Connect(function()
        ...
        list = U.mk("Frame", {
            ...
            BackgroundColor3 = C.BG2, BackgroundTransparency = 0.05,
            BorderSizePixel = 0, ZIndex = 300, Parent = h,
        })
        U.rc(list, 6) U.st(list, C.Gray4, 1, 0.4)
        U.pd(list, 4, 4, 4, 4)
        for _, opt in ipairs(options) do
            local ob = U.mk("TextButton", {
                Size = UDim2.new(1, 0, 0, 24), BackgroundColor3 = C.BG3,
                BackgroundTransparency = 0.3, BorderSizePixel = 0,
                Text = opt, TextColor3 = C.OffWhite, Font = F.Med,
                TextSize = FS.Small, AutoButtonColor = false, ZIndex = 301, Parent = list,
            })
            U.rc(ob, 4)
...
local function makeButton(parent, label, color, callback)
    ...
    local b = U.mk("TextButton", {
        Size = UDim2.new(1, 0, 0, 42), BackgroundColor3 = C.BG3,
        BackgroundTransparency = 0.2, BorderSizePixel = 0,
    ...
    U.mk("Frame", {
        ...
        BackgroundColor3 = color, BorderSizePixel = 0, Parent = b,
    })

    U.mk("TextLabel", {
        Size = UDim2.new(1, -30, 1, 0), Position = UDim2.new(0, 24, 0, 0),
        BackgroundTransparency = 1, Text = label, TextColor3 = C.OffWhite,
        Font = F.Bold, TextSize = FS.Body, TextXAlignment = Enum.TextXAlignment.Left, Parent = b,
    })
...
local function makeInfoRow(parent, label, value, color)
    local r = U.mk("Frame", { Size = UDim2.new(1, 0, 0, 24), BackgroundTransparency = 1, Parent = parent })
    U.mk("TextLabel", {
        Size = UDim2.new(0.5, 0, 1, 0), BackgroundTransparency = 1,
        Text = label, TextColor3 = C.Gray2, Font = F.Med, TextSize = FS.Small,
    ...
    local v = U.mk("TextLabel", {
        ...
        BackgroundTransparency = 1, Text = value, TextColor3 = color or C.OffWhite,
        Font = F.Bold, TextSize = FS.Small, TextXAlignment = Enum.TextXAlignment.Right, Parent = r,
    })
    return v
end
...
local function makeTextbox(parent, label, key, placeholder, callback)
    ...
    local tb = U.mk("TextBox", {
        Size = UDim2.new(1, -28, 0, 28), Position = UDim2.new(0, 14, 0, 28),
        BackgroundColor3 = C.BG4, BorderSizePixel = 0,
        Text = S[key] or "", PlaceholderText = placeholder or "...",
        PlaceholderColor3 = C.Gray3, TextColor3 = C.OffWhite,
        Font = F.Med, TextSize = FS.Small,
        TextXAlignment = Enum.TextXAlignment.Left, ClearTextOnFocus = false, Parent = h,
    })
    U.rc(tb, 6) U.pd(tb, 0, 0, 8, 8)
...
local function buildLogin()
    local Screen = U.mk("ScreenGui", {
        Name = "BMXLogin", ResetOnSpawn = false,
        IgnoreGuiInset = true, DisplayOrder = 1000,
    }, PlayerGui)

    U.mk("Frame", {
        Size = UDim2.new(1, 0, 1, 0), BackgroundColor3 = C.BG0,
        BackgroundTransparency = 0.2, BorderSizePixel = 0, Parent = Screen,
    })

    local Panel = U.mk("Frame", {
        Size = UDim2.new(0, 340, 0, 340), Position = UDim2.new(0.5, -170, 0.5, -170),
        BackgroundColor3 = C.BG1, BackgroundTransparency = 0.05,
        BorderSizePixel = 0, Parent = Screen,
    })
    U.rc(Panel, 16) U.st(Panel, C.Gray4, 1, 0.4)

    local top = U.mk("Frame", {
        Size = UDim2.new(1, 0, 0, 3), BackgroundColor3 = C.White,
        BorderSizePixel = 0, Parent = Panel,
    })
    U.rc(top, 16)

    U.mk("TextLabel", {
        Size = UDim2.new(1, 0, 0, 30), Position = UDim2.new(0, 0, 0, 30),
        BackgroundTransparency = 1, Text = "BLACK MATRIX", TextColor3 = C.White,
        Font = F.Black, TextSize = 26, TextXAlignment = Enum.TextXAlignment.Center, Parent = Panel,
    })

    U.mk("TextLabel", {
        Size = UDim2.new(1, 0, 0, 16), Position = UDim2.new(0, 0, 0, 62),
        BackgroundTransparency = 1, Text = "v" .. CFG.VERSION .. "  |  " .. CFG.GAME,
        TextColor3 = C.Gray2, Font = F.Norm, TextSize = 11,
        TextXAlignment = Enum.TextXAlignment.Center, Parent = Panel,
    })

    U.mk("Frame", {
        Size = UDim2.new(1, -50, 0, 1), Position = UDim2.new(0, 25, 0, 95),
        BackgroundColor3 = C.Gray4, BorderSizePixel = 0, Parent = Panel,
    })

    U.mk("TextLabel", {
        Size = UDim2.new(1, -50, 0, 16), Position = UDim2.new(0, 25, 0, 115),
        BackgroundTransparency = 1, Text = "USERNAME", TextColor3 = C.Gray1,
        Font = F.Bold, TextSize = 11, TextXAlignment = Enum.TextXAlignment.Left, Parent = Panel,
    })

    local userBox = U.mk("TextBox", {
        Size = UDim2.new(1, -50, 0, 38), Position = UDim2.new(0, 25, 0, 135),
        BackgroundColor3 = C.BG3, BackgroundTransparency = 0.2,
        BorderSizePixel = 0, PlaceholderText = "Masukkan username...",
        PlaceholderColor3 = C.Gray3, TextColor3 = C.White,
        Font = F.Med, TextSize = 13, TextXAlignment = Enum.TextXAlignment.Left,
        ClearTextOnFocus = false, Parent = Panel,
    })
    U.rc(userBox, 8) U.pd(userBox, 0, 0, 12, 12)

    U.mk("TextLabel", {
        Size = UDim2.new(1, -50, 0, 16), Position = UDim2.new(0, 25, 0, 185),
        BackgroundTransparency = 1, Text = "KEY", TextColor3 = C.Gray1,
        Font = F.Bold, TextSize = 11, TextXAlignment = Enum.TextXAlignment.Left, Parent = Panel,
    })

    local keyBox = U.mk("TextBox", {
        Size = UDim2.new(1, -50, 0, 38), Position = UDim2.new(0, 25, 0, 205),
        BackgroundColor3 = C.BG3, BackgroundTransparency = 0.2,
        BorderSizePixel = 0, PlaceholderText = "Masukkan key...",
        PlaceholderColor3 = C.Gray3, TextColor3 = C.White,
        Font = F.Med, TextSize = 13, TextXAlignment = Enum.TextXAlignment.Left,
        ClearTextOnFocus = false, Parent = Panel,
    })
    U.rc(keyBox, 8) U.pd(keyBox, 0, 0, 12, 12)

    local statusLbl = U.mk("TextLabel", {
        Size = UDim2.new(1, -50, 0, 16), Position = UDim2.new(0, 25, 0, 250),
        BackgroundTransparency = 1, Text = "", TextColor3 = C.Gray2,
        Font = F.Med, TextSize = 11, TextXAlignment = Enum.TextXAlignment.Center, Parent = Panel,
    })

    local loginBtn = U.mk("TextButton", {
        Size = UDim2.new(1, -50, 0, 44), Position = UDim2.new(0, 25, 0, 278),
        BackgroundColor3 = C.White, BorderSizePixel = 0,
        Text = "LOGIN", TextColor3 = C.BG0, Font = F.Black, TextSize = 14,
        AutoButtonColor = false, Parent = Panel,
    })
    U.rc(loginBtn, 8)

    U.mk("TextLabel", {
        Size = UDim2.new(1, 0, 0, 14), Position = UDim2.new(0, 0, 1, -22),
        BackgroundTransparency = 1, Text = "by " .. CFG.CREATOR .. "  |  " .. CFG.DISCORD,
        TextColor3 = C.Gray3, Font = F.Norm, TextSize = 10,
        TextXAlignment = Enum.TextXAlignment.Center, Parent = Panel,
    })
...
local function buildMainUI()
    ...
    local FAB = U.mk("Frame", {
        Size = UDim2.new(0, 52, 0, 52), Position = UDim2.new(1, -70, 0.5, -26),
        BackgroundColor3 = C.BG2, BackgroundTransparency = 0.1,
        BorderSizePixel = 0, Active = true, ZIndex = 200, Parent = Screen,
    })
    U.rc(FAB, 26) U.st(FAB, C.Gray4, 1, 0.3)
    U.mk("TextLabel", {
        Size = UDim2.new(1, 0, 1, 0), BackgroundTransparency = 1,
        Text = "B", TextColor3 = C.White, Font = F.Black, TextSize = 20,
        ZIndex = 201, Parent = FAB,
    })

    -- Panel
    local Panel = U.mk("Frame", {
        Size = UDim2.new(0, MAIN_W, 0, MAIN_H), Position = UDim2.new(0.5, -MAIN_W / 2, 0.5, -MAIN_H / 2),
        BackgroundColor3 = C.BG1, BackgroundTransparency = 0.15,
        BorderSizePixel = 0, Visible = false, Active = true, Draggable = true,
        ZIndex = 100, Parent = Screen,
    })
    U.rc(Panel, 14) U.st(Panel, C.Gray4, 1, 0.5)
    ...
    U.mk("TextLabel", {
        Size = UDim2.new(0, 200, 0, 20), Position = UDim2.new(0, 14, 0, 8),
        BackgroundTransparency = 1, Text = "BLACK MATRIX", TextColor3 = C.White,
        Font = F.Black, TextSize = 15, TextXAlignment = Enum.TextXAlignment.Left,
        ZIndex = 102, Parent = Header,
    })
    U.mk("TextLabel", {
        Size = UDim2.new(0, 200, 0, 14), Position = UDim2.new(0, 14, 0, 26),
        BackgroundTransparency = 1, Text = "Violence District", TextColor3 = C.Gray2,
        Font = F.Norm, TextSize = 10, TextXAlignment = Enum.TextXAlignment.Left,
        ZIndex = 102, Parent = Header,
    })
    U.mk("TextLabel", {
        Size = UDim2.new(0, 220, 0, 14), Position = UDim2.new(0, 130, 0, 16),
        BackgroundTransparency = 1, Text = "| " .. CFG.DISCORD, TextColor3 = C.Cyan,
        Font = F.Med, TextSize = 10, TextXAlignment = Enum.TextXAlignment.Left,
        ZIndex = 102, Parent = Header,
    })

    local minBtn = U.mk("TextButton", {
        Size = UDim2.new(0, 32, 0, 32), Position = UDim2.new(1, -78, 0, 7),
        BackgroundColor3 = C.BG3, BackgroundTransparency = 0.3,
        BorderSizePixel = 0, Text = "-", TextColor3 = C.Gray1,
        Font = F.Bold, TextSize = 15, AutoButtonColor = false,
        ZIndex = 103, Parent = Header,
    })
    U.rc(minBtn, 8)
    ...
    local closeBtn = U.mk("TextButton", {
        Size = UDim2.new(0, 32, 0, 32), Position = UDim2.new(1, -42, 0, 7),
        BackgroundColor3 = C.BG3, BackgroundTransparency = 0.3,
        BorderSizePixel = 0, Text = "X", TextColor3 = C.Red,
        Font = F.Bold, TextSize = 13, AutoButtonColor = false,
        ZIndex = 103, Parent = Header,
    })
    U.rc(closeBtn, 8)
    ...
    local Search = U.mk("Frame", {
        Size = UDim2.new(0, SIDEBAR_W - 20, 0, 30), Position = UDim2.new(0, 10, 0, 54),
        BackgroundColor3 = C.BG2, BackgroundTransparency = 0.3,
        BorderSizePixel = 0, ZIndex = 101, Parent = Panel,
    })
    U.rc(Search, 8)
    U.mk("TextLabel", {
        Size = UDim2.new(0, 16, 1, 0), Position = UDim2.new(0, 8, 0, 0),
        BackgroundTransparency = 1, Text = "?", TextColor3 = C.Gray2,
        Font = F.Bold, TextSize = 12, ZIndex = 102, Parent = Search,
    })
    U.mk("TextBox", {
        Size = UDim2.new(1, -30, 1, 0), Position = UDim2.new(0, 26, 0, 0),
        BackgroundTransparency = 1, Text = "", PlaceholderText = "Search...",
        PlaceholderColor3 = C.Gray3, TextColor3 = C.OffWhite,
        Font = F.Norm, TextSize = 11, TextXAlignment = Enum.TextXAlignment.Left,
        ClearTextOnFocus = false, ZIndex = 102, Parent = Search,
    })

    -- Sidebar (FIXED - pakai CanvasGroup biar gak nutup)
    local Sidebar = U.mk("ScrollingFrame", {
        Size = UDim2.new(0, SIDEBAR_W - 20, 1, -110), Position = UDim2.new(0, 10, 0, 92),
        BackgroundTransparency = 1, BorderSizePixel = 0,
        ScrollBarThickness = 2, ScrollBarImageColor3 = C.Gray3,
        CanvasSize = UDim2.new(0, 0, 0, 300),
        ZIndex = 150, Parent = Panel,
    })
    U.ls(Sidebar, 4)

    -- Content (FIXED - CanvasSize manual)
    local Content = U.mk("ScrollingFrame", {
        Size = UDim2.new(1, -SIDEBAR_W - 20, 1, -60), Position = UDim2.new(0, SIDEBAR_W + 10, 0, 54),
        BackgroundTransparency = 1, BorderSizePixel = 0,
        ScrollBarThickness = 3, ScrollBarImageColor3 = C.Gray3,
    ...
    local PageTitle = U.mk("TextLabel", {
        Size = UDim2.new(1, -SIDEBAR_W - 20, 0, 24), Position = UDim2.new(0, SIDEBAR_W + 10, 0, 26),
        BackgroundTransparency = 1, Text = "Info", TextColor3 = C.White,
        Font = F.Black, TextSize = 18, TextXAlignment = Enum.TextXAlignment.Left,
        ZIndex = 102, Parent = Panel,
    })

    -- ═══ PAGES (pakai array, bukan dict) ═══
    local pageList = {}
    ...
    local function switchPage(targetName)
        print("[BMX] === Switching to: " .. targetName .. " ===")
        ...
        for _, entry in ipairs(pageList) do
            if entry.name == targetName then
                ...
                print("[BMX] Showed: " .. entry.name .. " | Height: " .. tostring(totalH))
                break
            end
        end
    ...
    local infoBox = U.mk("Frame", { Size = UDim2.new(1, 0, 0, 130), BackgroundTransparency = 1, Parent = infoContent })
    U.ls(infoBox, 4)
    ...
    U.mk("TextLabel", {
        ...
        TextColor3 = C.Gray1, Font = F.Norm, TextSize = 11,
        TextWrapped = true, TextXAlignment = Enum.TextXAlignment.Left,
        TextYAlignment = Enum.TextYAlignment.Top, Parent = warnContent,
    })

    local _, liveContent = makeCard(pageInfo, "LIVE DATA", C.Green, false)
    local liveBox = U.mk("Frame", { Size = UDim2.new(1, 0, 0, 130), BackgroundTransparency = 1, Parent = liveContent })
    U.ls(liveBox, 4)
    ...
    local IHit = makeInfoRow(liveBox, "Hits", "0", C.Red)
    ...
    U.mk("TextLabel", {
        ...
        Text = "Normal = safe zone | Perfect = ideal hit timing",
        TextColor3 = C.Gray2, Font = F.Norm, TextSize = 10,
        TextWrapped = true, TextXAlignment = Enum.TextXAlignment.Left, Parent = genContent,
    })

    local _, aimContent = makeCard(pageSurv, "AIM SILENT (TOF)", C.Red, false)
    makeToggle(aimContent, "Aim Silent", "AimSilent")
    makeDropdown(aimContent, "Target", {"Killer", "Survivor", "Zombie", "All"}, "AimTarget")
    makeDropdown(aimContent, "Aim Part", {"Head", "Body"}, "AimPart")
    ...
    makeToggle(aimContent, "Visible Only", "AimVisibleOnly")

    local _, tofContent = makeCard(pageSurv, "TOF SYSTEM", C.Orange, false)
    makeToggle(tofContent, "TOF", "TOF")
    ...
    local _, escContent = makeCard(pageSurv, "AUTO ESCAPE", C.Yellow, false)
    makeToggle(escContent, "Auto Escape", "AutoEscape")

    -- ═══ PAGE: KILLER ═══
    local _, kAimContent = makeCard(pageKiller, "KILLER AIM (VEIL)", C.Red, true)
    ...
    local _, kSpearContent = makeCard(pageKiller, "SPEAR CONTROL", C.Orange, false)
    ...
    -- ═══ PAGE: VISUAL ═══
    local _, espContent = makeCard(pageVisual, "ESP (OUTLINE ONLY)", C.Pink, true)
    ...
    makeButton(utilContent, "Reset Stats", C.Red, function()
        S._hitCount = 0 S._genCount = 0 S._parryCount = 0 S._aimHits = 0
    ...
    makeButton(cfgContent, "Delete Config", C.Red, function()
        pcall(function()
            if delfile and isfile("BMX_config.json") then
                delfile("BMX_config.json")
    ...
    U.mk("TextLabel", {
        ...
        TextColor3 = C.Gray1, Font = F.Norm, TextSize = 11,
        TextWrapped = true, TextXAlignment = Enum.TextXAlignment.Left,
        TextYAlignment = Enum.TextYAlignment.Top, Parent = credContent,
    })

    -- ═══ SIDEBAR BUTTONS ═══
    local function addSidebarButton(pageName)
        local btn = U.mk("TextButton", {
            Size = UDim2.new(1, 0, 0, 42), BackgroundColor3 = C.BG2,
            BackgroundTransparency = 1, BorderSizePixel = 0,
            Text = "", AutoButtonColor = false, ZIndex = 151, Parent = Sidebar,
        })
        U.rc(btn, 6)

        local ind = U.mk("Frame", {
            Size = UDim2.new(0, 3, 0, 0), Position = UDim2.new(0, 0, 0.5, 0),
            BackgroundColor3 = C.White, BorderSizePixel = 0, Parent = btn,
        })
        U.rc(ind, 2)

        local lbl = U.mk("TextLabel", {
            Size = UDim2.new(1, -20, 1, 0), Position = UDim2.new(0, 20, 0, 0),
            BackgroundTransparency = 1, Text = pageName, TextColor3 = C.Gray2,
            Font = F.Med, TextSize = FS.Body,
...
UserInputService.InputBegan:Connect(function(input, gp)
    ...
    elseif k == Enum.KeyCode.F7 then
        ...
        S.KillerAim = false S.KillerAutoAttack = false S.SpeedBoost = false
        S.Fly = false S.AutoEscape = false
        flyDisable()
...
LocalPlayer.Chatted:Connect(function(msg)
    ...
    elseif msg == "!parry" then S.AutoParry = not S.AutoParry notify("Parry", tostring(S.AutoParry), C.Cyan)
    elseif msg == "!aim" then S.AimSilent = not S.AimSilent notify("Aim", tostring(S.AimSilent), C.Red)
    elseif msg == "!scan" then RC = {} notify("Scan", scanRemotes() .. " cached", C.Purple)
    elseif msg == "!panic" then
        S.AimSilent = false S.AutoGenerator = false S.AutoParry = false
        S.KillerAim = false S.KillerAutoAttack = false S.AutoEscape = false
        notify("PANIC", "All off", C.Red)
    elseif msg == "!esp" then
        S.ESPKiller = not S.ESPKiller S.ESPSurvivor = not S.ESPSurvivor
        notify("ESP", tostring(S.ESPKiller), C.Pink)
    end
end)
